...
public:
    ...
    BoxCollection(long x, long y, GameScene* gameScene, float size)
        :x(x),y(y),gameScene(gameScene),size(size),color(MyColor::RandomLight()),actual_box(this) {
    ...
    BoxCollection* copy(float size, long posX, long posY, BoxCollection* father, bool real);
    ...
    static BoxCollection* create(Color3B color, long x, long y, GameScene* gameScene,
        float size, long posX, long posY, Node* boxes[MAX_SIZE][MAX_SIZE], BoxCollection* father, BoxCollection* actual_box, bool real)
    {
        BoxCollection* pRet = new(std::nothrow) BoxCollection(color, x, y, gameScene,size,posX,posY,boxes,father, actual_box,real);
        if (pRet && pRet->init())
        {
            pRet->autorelease();
            return pRet;
